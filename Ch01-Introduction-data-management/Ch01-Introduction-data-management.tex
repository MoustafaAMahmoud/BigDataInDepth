...
\item  We have requirements or business case (It could be DWH unification)
\item  Identify the stakeholders (Data owner(s))
\item  Before we start, ALL communications from start to the end should be documented into any format.
	\begin{itemize}
		\item  Conflounce page, Word, Sheet.(The best option to be query format!!)
		\item  Make the discussion online and put comments to make the history available always (we need to stop the one man show).
		\item  All tasks should be clear what is the expected output for example (analysis means document what is the column names, structure format).
	\end{itemize}
\item  Source System Data Analysis.
\item  Check The data structure
\item  Check for the data latency
\item  Check for connectivity and security analysis (assessment).
\item  Technical discussion about the best way to ingest the data (push/pull).
\item  Data Ingestion format (file, database, api, etc..)
\item  Sign or confirmation for every point from the stakeholders.
\end{itemize}
