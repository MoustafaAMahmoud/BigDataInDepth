...
%	\item Data retention or \textbf{archiving} process \faEdit \space \faArrowCircleORight \space (Hot or Cold storage).
%\end{itemize}
%\end{itemize}
%
%\end{frame}
...
%				\item Could it help if we change the backend engine \faEdit \space \faArrowCircleORight \space (DBMS system)?
%			\end{itemize}			
%		\end{itemize}
...
%             \item There is a performance issue in the existing reports, and you need to change the model. \faEdit \space \faArrowCircleORight \space reduce the join by creating a new join table (\textit{materialized view}).
...
%\begin{itemize}[<+->]
%\item This company has other systems \faEdit \space \faArrowCircleORight \space billing, charging, signaling.	
...
%				\item It's functionality is based on the aggregate data not the transactions \faEdit \space \faArrowCircleORight \space (It needs the total outgoing calls till time or it needs the total charging amounts from prepaid or the available limits from billing if it is postpaid).
%			\end{itemize}
%	\end{itemize}
%
%\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item Segregation of this layer if it uses different storage type or multi-teams access this layer for a different purpose \faEdit \space \faArrowCircleORight \space (Kafka)\\ \red{\textit{\faBullhorn Kafka is not a storage layer but it could be used as landing layer or data persistence layer}}.
...
\begin{frame}
	...
	\begin{block}{Junk Dimension Table Size}
		\begin{itemize}
			...
			\item It is easy to calculate the expected number of rows as it is the total number of combinations between the low-cardinality attributes \\ \faEdit \space \faArrowCircleORight \space 3 columns each has 3 values total = 3 * 3 = 9.
		\end{itemize}
	\end{block}
