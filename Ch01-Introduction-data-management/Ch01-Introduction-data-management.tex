...
\begin{itemize}[<+->]
	\item Be familiar with data management life-cycle. \pause
	\item Introduction to data warehouse and its usage. \pause	
	\item Motivation to DWH.\pause
	\item What is the different types of DWH?\pause
	\item Use cases\pause
	\item Data Encoding and Formats\pause	
	\item Challenges to build a DWH.\pause
	\item Data modeling design.\pause
\end{itemize}
...
\end{definition}	
%Capacity of changing in one level without affecting the other levels.
	\begin{itemize}[<+->]
...
\begin{frame}
	...
	\begin{itemize}[<+->]
		...
		\begin{itemize}[<+->]
			\item Lowest level.
			\item Describes how data is stored (files, indices, etc. on the random access disk system).  
			\item Describes the data structure.
			\item Describes the record layout of files and type of files (hash, b-tree, flat).
			
		\end{itemize}		
		%https://beginnersbook.com/2015/04/levels-of-abstraction-in-dbms/
	\end{itemize}	
	\begin{example}
		Database contains product information. Physical layer describes storage mechanism and the blocks (bytes, gigabytes, terabytes etc.) and the amount of memory used. Usually this layer abstracted from the programmers.
	\end{example}
...
\begin{frame}
	...
	\begin{itemize}[<+->]
		...
		\begin{itemize}[<+->]
			...
			\item \textbf{\underline{\blue{What}}} is the data stored?  
