...
%             \item There is a performance issue in the existing reports, and you need to change the model. \faEdit reduce the join by creating a new join table (\textit{materialized view}).
...
%\begin{itemize}[<+->]
%\item This company has other systems \faEdit billing, charging, signaling.	
...
%				\item It's functionality is based on the aggregate data not the transactions \faEdit (It needs the total outgoing calls till time or it needs the total charging amounts from prepaid or the available limits from billing if it is postpaid).
%			\end{itemize}
%	\end{itemize}
%
%\end{frame}
...
\begin{frame}
    ...
    \begin{wideitemize}
        ...
        \begin{wideitemize}
            ...
            \item Non-Volatile: \textit{It refers to the data will not erased or deleted (It could be archived and retrieved when needed). Data can be accumulated daily the new snapshots (refreshed at based on the source system interval. \faEdit It could be updated daily, weekly, and monthly).}
        \end{wideitemize}
    \end{wideitemize}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            ...
            \item  All tasks should be clear what is the expected output \faEdit (analysis means to document data structure, format, column names, etc..).
        \end{itemize}
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \begin{itemize}[<+->]
            ...
            \item Hire an architect to put the architecture in more detailed way \faEdit the size for each room, the distribution of the wireds, where the plumbing fixtures will be placed, etc. (Architecture phase)
    ...
    \begin{description}[<+->]
        ...
        \begin{example}
            \begin{itemize}[<+->]
                ...
                ex: (upper/lower) we can create dimension table for the product details and use product id unified across fact tables.
            \end{itemize}
        \end{example}
    \end{description}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\midTitle{Dimensions Types: Junk Dimension}
\begin{frame}
...
\begin{frame}
    ...
    \begin{block}{Conformed vs Role-Playing}
        \begin{itemize}
            \item \textbf{Conformed} is the same dimension which used in different facts and has \textit{\underline{the same meaning}} ex: CustomerID.
            \item \textbf{Role-Playing} is the same dimension which used multiple times within the same fact but \textit{\underline{with different meanings}} ex: Date.
        \end{itemize}
    \end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item \blue{\textit{\faBullhorn This dimensions design should be used carefully without limited cases}}.
    \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item \red{\textit{\faBug This dimensions design is not recommended as it has a lot of complexity to the model and query performance. Also, it complicates the ETL process and make too much dimensions without needs}}.
    \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \red{\textit{\faBug This dimensions design is not recommended as it has a lot of complexity
        to the model and query performance. Also, it complicates the ETL process and make too much dimensions without needs}}.
    \end{itemize}
    \centering
...
\begin{frame}
	...
	\end{itemize}
		\begin{example}
			...
			\end{itemize}
		\end{example}
%https://www.kimballgroup.com/2011/08/design-tip-137-creating-and-managing-shrunken-dimensions/
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\frametitle{Attention: Conformed vs Role-Playing Dimension vs Swappable}
	\begin{itemize}[<+->]
		\item \textbf{Conformed} is the same dimension which used in different facts and has \textit{\underline{the same meaning and value}} \faEdit CustomerID 123 can be represented into the whole model using the same value and same meaning.
		\item \textbf{Role-Playing} is the same dimension which used multiple times within the same fact but \textit{\underline{with different meanings and same value}} \faEdit Date Dimension 20191012 can be used for different purpose order delivery date, expire date but different meanings.
		\item \textbf{Swappable} different version from the primary dimension each version has its own attributes and meanings based on the use case \textit{\underline{(different meaning based on the category)}}. \faEdit party id dimension has different version sales, agent, employee and all of them sub-types of the party but used for different purpose in different facts.
	\end{itemize}
