...
\begin{itemize}[<+->]
	\item Simplify the concepts in data management \blue{(Ch.2)}.
	\item Understand the data management life-cycle \blue{(Ch.2)}. 
	\item Illustrate the basics of distributed systems concepts \blue{(Ch.3)}.
	\item Be familiar with ETL for (batch/steaming) data over distributed systems ex: Hadoop \& Spark \blue{(Ch.2,3,4,6)}.  
	\item Apply QA and testing the data \blue{(Ch.6,7)}.
	\item Building real-life examples \blue{(Ch.7)}. 
	\item Build and scale your data product \blue{(Ch.7,8,9)}.    
	\item Applying machine learning over big data \blue{(Ch.6,7)}. 
	\item Automate the data life-cycle process end-to-end (e2e) \blue{(Appx. H)}. 
	\item Understanding of the DevOps tools and functions in data life-cycle \blue{(Appx. H)}.
\end{itemize}
...
\begin{frame}[t]
\frametitle{Chapters Dependencies}
...
\begin{frame}
	...
	\node (start)             [required]              {Ch.01 Introduction};
	\node (pro2a) [flags, left of=start, xshift=-5cm] {\faBug \space You MUST finish the\\ red chapters first};
	\node (pro2a2) [note, right of=start, xshift=5cm] {\faBell \space Finish colors group \\before moving to the next.};
